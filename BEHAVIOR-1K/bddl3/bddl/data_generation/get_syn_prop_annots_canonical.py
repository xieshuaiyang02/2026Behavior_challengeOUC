...
CANONICAL_FN = (
    ...
    / "generated_data"
    / "syn_prop_annots_canonical.json"
)
PROP_TO_SYN_FILE = (
    pathlib.Path(__file__).parents[1] / "generated_data" / "properties_to_synsets.json"
)  # gives all applicable synsets for a given property
SYN_TO_DESC_FILE = (
    pathlib.Path(__file__).parents[1] / "generated_data" / "synsets_to_descriptors.json"
)  # gives states for a given synset

DESIRED_PROPS = set(
    pcfg.CROWD_PROP_NAMES + pcfg.GPT_PROP_NAMES + pcfg.INTERNAL_PROP_NAMES
)
...
def add_programmatic_properties(
    synset_content,
):  # runs programmatic addition over existing canonical input
    ...
    for synset in synset_content:
        ...
        if (
            ("visualSubstance" in synset_content[synset])
            or ("microPhysicalSubstance" in synset_content[synset])
            or ("macroPhysicalSubstance" in synset_content[synset])
            or ("liquid" in synset_content[synset])
        ):
            synset_content[synset]["substance"] = {}
        if (
            ("microPhysicalSubstance" in synset_content[synset])
            or ("macroPhysicalSubstance" in synset_content[synset])
            or ("liquid" in synset_content[synset])
        ):
            synset_content[synset]["physicalSubstance"] = {}
        if (
            ("cloth" in synset_content[synset])
            or ("rope" in synset_content[synset])
            or ("softBody" in synset_content[synset])
        ):
        ...
        if ("nonSubstance" in synset_content[synset]) and (
            "cookable" in synset_content[synset] or "fillable" in synset_content[synset]
        ):
        ...
        if (
            "nonSubstance" in synset_content[synset]
        ):  # non-substances are both wetable and mixable
...
def get_synset_descriptors(synsets_to_filtered_properties):  # take in canonical format
    ...
    for synset_label, properties in synsets_to_filtered_properties.items():
        synsets_to_states[synset_label] = [
            [PROP_TO_DESC[prop]] * 2
            for prop in properties
            if PROP_TO_DESC[prop] is not None
        ]  # if the property has an attached state, add its states
