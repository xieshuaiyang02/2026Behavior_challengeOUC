"""OUC server using the official JAX policy and B1K websocket interface."""
...
from openpi.policies import policy_config_ouc as _policy_config
from openpi.serving import websocket_b1k_server
from openpi.shared.eval_b1k_wrapper import B1KPolicyWrapper
from openpi.training import config_ouc as _config
...
def main(args: Args) -> None:
    ...
    if not 1 <= args.action_horizon <= policy_metadata["action_horizon"]:
        raise ValueError("Executed action horizon exceeds checkpoint prediction horizon")
    ...
    policy = B1KPolicyWrapper(
        ...
        max_len=policy_metadata["action_horizon"],
    )
