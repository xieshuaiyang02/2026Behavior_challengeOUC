"""OUC extension of this repository's official B1K JAX/Flax training entry.

Keeps the B1K validation FM loss, optimizer, FSDP, EMA and checkpoint schedule.
"""
...
from openpi.models.pi0_ouc import compute_stage_losses
import openpi.shared.array_typing as at
import openpi.shared.nnx_utils as nnx_utils
import openpi.training.checkpoints_ouc as _checkpoints
import openpi.training.config_ouc as _config
import openpi.training.data_loader as _data_loader
import openpi.training.data_loader_ouc as _stage_data_loader
...
def _prepare_validation_config(
    config: _config.TrainConfigOUC, training_norm_stats: dict | None
) -> tuple[_config.TrainConfigOUC, str, bool]:
    """
...
def _create_validation_data_loader(
    ...
    val_config: _config.TrainConfigOUC,
    *,
    use_norm_stats: bool,
...
def init_wandb(config: _config.TrainConfigOUC, *, resuming: bool, log_code: bool = False, enabled: bool = True):
    if not enabled:
...
def init_train_state(
    config: _config.TrainConfigOUC, init_rng: at.KeyArrayLike, mesh: jax.sharding.Mesh, *, resume: bool
...
def train_step(
    ...
    def loss_fn(model, rng, observation, actions, targets):
        output = model.compute_outputs(rng, observation, actions, train=True)
        losses = compute_stage_losses(output, targets, config.model)
        return losses["loss"], losses

    train_rng = jax.random.fold_in(rng, state.step)
    observation, actions, targets = batch
    diff_state = nnx.DiffState(0, config.trainable_filter)
    (loss, stage_info), grads = nnx.value_and_grad(loss_fn, argnums=diff_state, has_aux=True)(
        model, train_rng, observation, actions, targets
    )
...
def main(config: _config.TrainConfigOUC):
    init_logging()
    config = _config.resolve_stage_config(config)
    ...
    data_loader = _stage_data_loader.create_data_loader(
